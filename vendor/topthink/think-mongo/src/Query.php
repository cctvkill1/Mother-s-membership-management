...
class Query
{
     ...
     * 聚合查询
     * @access public
     * @param array $aggregate 聚合指令 
     * @return mixed
     */
    public function aggregate($aggregate)
    { 
        $result = $this->cmd('aggregate', $aggregate);
        return isset($result[0]['result']) ? $result[0]['result'] :  $result;
    }
     ...
     * @throws BulkWriteException
     */
    public function insert(array $data, $replace = null, $getLastInsID = true)
    {
        if (empty($data)) {
     ...
     * @throws BulkWriteException
     */
    public function update(array $data,array $upsert=[])
    {
        $options = $this->parseExpress(); 
        if($upsert)
